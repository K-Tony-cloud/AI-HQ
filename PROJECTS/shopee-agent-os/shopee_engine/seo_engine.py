...
_PRODUCT_TYPE_RULES: list[dict] = [
    {
        ...
        # Hard blocks — unambiguous adapter/charger-only signals
        "block_title": [
            "adapter", "อะแดปเตอร์", "หัวชาร์จ", "wall charger",
        ],
        # Soft blocks — contextually ambiguous; exempted when built-in cable or PB evidence present
        "context_sensitive_block": [
            ...
            (
                "cable",
                # Exempt when cable appears as connector-type descriptor inside a Power Bank listing
                # e.g. "Type C และ L-Cable 2.4A" — the cable IS the built-in feature
                ["สายในตัว", "สายชาร์จในตัว", "powerbank", "power bank",
                 "พาวเวอร์แบงค์", "แบตสำรอง", "built-in"],
            ),
            (
                "lightning cable",
                # Exempt when "lightning cable" describes the built-in cable connector
                ["สายในตัว", "สายชาร์จในตัว", "powerbank", "แบตสำรอง"],
            ),
        ],
...
def _build_product_blocks(
    ...
    show_ccc_evidence: when True, render CCC evidence badge under each product.
    """
    ...
    for i, p in enumerate(products, 1):
        ...
        # CCC evidence badge (CCC articles only)
        if show_ccc_evidence:
            ev_source = p.get("ccc_evidence_source", "no_evidence")
            ev_text   = p.get("ccc_evidence_text", "")
            ev_label, ev_note = _EVIDENCE_LABELS.get(ev_source, _EVIDENCE_LABELS["no_evidence"])
            ev_snippet = f' — `{ev_text[:40]}`' if ev_text else ""
            block += f'<span class="ccc-evidence {ev_source}">{ev_label}{ev_snippet} ({ev_note})</span>\n\n'
...
def validate_article_for_review(article_id: str) -> dict:
    ...
    # --- Variant Price / Spec Plausibility Gate (blocking error for implausible; warning for multi-variant at rank 1) ---
    if not products.empty:
        ...
        for _, prow in products.iterrows():
            ...
            prank        = int(prow.get("rank_in_article") or 0)
            ...
            if not plaus_ok:
                # Blocking error — price is implausible for the claimed spec
                errors.append(
                    f"itemid {pitemid}: {plaus_reason}. "
                    f"เปลี่ยนสินค้าด้วย /seo-product-remove และ /seo-product-add"
                )
            elif plaus_ev.get("is_multivariant") and prank == 1:
                # Warning only: rank-1 product is multi-variant — user should verify correct SKU
                warnings.append(
                    f"itemid {pitemid} (rank 1): multi-variant listing — "
                    f"ตรวจ affiliate link ว่าชี้ไปที่ variant ที่ตรงกับบทความ ก่อน publish"
                )
